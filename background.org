...
** Norms
*** Distances
A distance (or metric) $d$ on a space $X$ satisfies
- $d(x,x) = 0$
- $d(x,y) > 0$ if $x \neq y$.
...
**** Absolute value
For any $x \in \Reals$, $|x| = x$ if $x > 0$ and $|x| = - x$ if $x < 0$.

**** $L_p$ norm
For any vector $x \in \Reals^n$,
\[
\|x\|_p \defn \left(\sum_{i=1}^n |x_i|^p\right)^{1/p}
\]
Special cases: $L_1, L_2, L_\infty$.
...
\[
\|x\|_2 = \sqrt{\left(\sum_{i=1}^n (x_i)^p\right)^{1/p}}
\]
...
** Functional analysis
*** Lipschitz functions
A function $f : X \to Y$ is Lipschitz with respect to some distances $d, \rho$ on $X, Y$ if
\[
