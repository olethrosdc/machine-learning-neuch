...
\[
f_\param(\bx_t) = \param^\top \bx_{t} = \sum_{i=1}^n \param_i x_{t,i}
\]
...
\[
\min_\param \sum_{t=1}^T [y_t - f_\param(\bx_t)]^2
\]
...
p_\param(y_t| \bx_t) 
\propto
\exp(-[y_t - f_\param(\bx_t)]^2 / 2\sigma^2)
\]
Maximising the log-likelihood is equivalent to minimising mean-squared error:
\[
\argmax_\param \sum \ln p_\param(y_t| \bx_t) = \argmin_\param \sum_t |y_t - f_\param(\bx_t)|^2
\]
...
&= \nabla \sum_{t=1}^T [y_t - \pi_\param(\bx_t)]^2
\\
&= \sum_{t=1}^T \nabla [y_t - \pi_\param(\bx_t)]^2
\\
&= \sum_{t=1}^T 2 [y_t - \pi_\param(\bx_t)] [- \nabla \pi_\param(\bx_t)]^2
\end{align*}
**** Parameter gradient
For a linear regressor:
\[
\frac{\partial}{\partial \param_j} \pi_\param(\bx_t) = x_{t,j}.
\]
