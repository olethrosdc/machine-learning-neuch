...
\[
\E[\exp(\lambda x)] \leq \exp(\lambda^2\sigma^2/2), \qquad \forall \lambda \in \Reals.
\]
...
\begin{align*}
\Pr(x \geq 0) &= \Pr(\exp(\lambda x) \geq \exp(\lambda \epsilon))\\
...
*** Exercise
- Show that if $\E[x] = 0$ and $X \in [a,b]$ then x is $(b -a)/2$ subgaussian.
- [hard]
