...
- The alternative $H_1$ is that  $x_t \sim \Ber(\theta)$, with $\theta \neq 1/2$.
...
\]
 However, this tells us nothing about $\Pr(S_0 \mid H_1)$. Using Hoeffding's inequality, show for which values of $\theta \neq \frac{1}{2}$, we have that $\Pr(S_0 \mid H_1) \leq \alpha$, i.e.
\[
\Pr(|\hat{\mu}_T - 0.5| < \tau \mid \theta) \leq \alpha
\]
