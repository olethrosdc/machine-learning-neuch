...
\end{align}
*** Chernoff bound
The Chernoof bound uses the fact that if $x \geq y$ is true, then $f(x) \geq f(y)$ for any monotonic increasing function $f$. In particular:
\[
\Pr(x - \mu \geq u) 
=
...
\]
*** Normal bound
We can use the above tail bound to prove a bound on the error of the empirical estimate $\hat{\mu}$ of a normal with mean $\mu$ after observing $T$ samples.
...
**** Subgaussianity                                            :B_definition:
:PROPERTIES:
:BEAMER_env: definition
:END:
$x$ is \sigma-subgaussian if
$\E[\exp(\lambda x)] \leq \exp(\lambda^2\sigma^2/2), \qquad \forall \lambda \geq 0$.

**** Subgaussian bound :B_theorem:
:PROPERTIES:
:BEAMER_env: theorem
:END:
If $x$ is \sigma-subgaussian then, for all $\epsilon \geq 0$,                
\begin{equation}
...
\begin{align*}
\Pr(x \geq \epsilon) &= \Pr(\exp(\lambda x) \geq \exp(\lambda \epsilon))
\leq \E[\exp(\lambda x)] \exp(- \lambda \epsilon)\\
...
- If $x$ is $\sigma$ subgaussian then $\E[x] = 0$, $\Var[x] \leq \sigma^2$
- $cX$ is $|c|\sigma$ subgaussian for all $c \in \Reals$.
- If $x_1, x_2$ are $\sigma_1, \sigma_2$ subgaussian then
$x_1 + x_2$ is $\sqrt{\sigma_1^2 + \sigma_2^2}$ subgaussian.

The above facts lead to the following corollary:
**** Corollary
If $x_t - \mu$ are independent $\sigma$ subgaussian and
\[
\hat{\mu} = \frac{1}{T} \sum_{t=1}^T x_t
\]
\[
\Pr(\hat{\mu} \geq \mu + \epsilon) \leq \exp\left(-\frac{n\epsilon^2}{2\sigma^2}\right)
\]
...
- If $x \in [a,b]$ then it is $(b - a)/2$ subgaussian. This directly leads to the following inequality:\footnote{There other ways of proving it, but this is the easiest given the previous development}
...
it holds for the empirical mean $\hat{\mu}_T = \frac{1}{T} \sum_{t=1}^T x_t$:
\[
\Pr(|\mu - \hat{\mu}_T| \geq \epsilon) \leq 2 \exp\left( -2T \epsilon^2\right)
\]
...
- [easy] If $x$ is $\sigma$ subgaussian, then $cx$ is $c\sigma$ subgaussian
- [medium] If $x_i$ are $\sigma$ subgaussian then $\sum_i x_i$ is $\sqrt{\sum_i \sigma_i^2}$ subgaussian
- [hard] if $\E[x] = 0$ and $x \in [a,b]$ then x is $(b -a)/2$ subgaussian.
...
- The null hypothesis $H_0$ is that $x_t \sim \Ber(1/2)$.
- The alternative $H_1$ is that  $x_t \sim \Ber(\mu)$, with $\mu \neq 1/2$.
- You have a statistical test which, for any significance level $\alpha \in [0,1]$, returns $S_1$ when $H_0$ with probability $\alpha$. 
  This is implemented by choosing a threshold $\tau$ so that
\[
\Pr(|\hat{\mu}_T - 0.5| \geq \tau \mid H_0) = \alpha.
\]
 However, this tells us nothing about $\Pr(S_0 \mid H_1)$. Using Hoeffding's inequality, show for which values of $\mu \neq \frac{1}{2}$, we have that $\Pr(S_0 \mid H_1) \leq \alpha$, i.e.
\[
\Pr(|\hat{\mu}_T - 0.5| < \tau \mid \mu) \leq \alpha
\]
Hint: Define $\Delta = |\mu - \frac{1}{2}|$. Show that, this holds when $\Delta > \sqrt{\ln(2/\alpha)/2T} + \tau$.
