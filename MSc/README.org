...
|--------+-------+--------------------------------+-----------------------------------------------------+---------------------+---------|
|      1 | 09.17 | Introduction                   | Mean estimation, Classification, kNN                | nympy, pandas       | Lecture |
|      2 | 09.24 | Learning and Generalisation    | Train/Test, XV, Bootstrapping, Simulation           | pandas, scikitlearn | Mixed   |
|--------+-------+--------------------------------+-----------------------------------------------------+---------------------+---------|
|      3 | 10.01 | Classification, Perceptron     | Margin, Regret, Gradient Descent                    | scikit learn        | Mixed   |
|      6 | 10.08 | Neural Networks                | Logistic Regression, Backpop, Stochastic GD         | pyTorch intro       | Mixed   |
|      5 | 10.15 | Neural Network Lab             | Project discussion, Linear Regression Lab           | pyTorch             | Lab     |
|      4 | 10.22 | Regression, Linear             | Linear regression, Least Squares, Probabilistic     | pyTorch             | Mixed   |
|--------+-------+--------------------------------+-----------------------------------------------------+---------------------+---------|
|      7 | 10.29 | Probabilistic models           | ML, MAP, Bayes, Beta-Bernoulli, Hypothesis testing  |                     | Lecture |
|      8 | 11.05 | Generative modelling           | Bayesian Networks, Markov Models, Tree Models, CRFs |                     | Mixed   |
|      9 | 11.12 | Approximate Bayesian Inference | Markov Chain Monte Carlo                            |                     | Misc    |
|--------+-------+--------------------------------+-----------------------------------------------------+---------------------+---------|
|     10 | 11.19 | Learning Theory                | Chernoff bounds (Perceptron II)                     |                     | Lecture |
|     11 | 11.26 | Fairness                       |                                                     |                     | Mixed   |
|     12 | 12.03 | Reinforcement Learning         |                                                     |                     | Mixed   |
|--------+-------+--------------------------------+-----------------------------------------------------+---------------------+---------|
|     13 | 12.10 | Lab                            |                                                     |                     | EXAM    |
|--------+-------+--------------------------------+-----------------------------------------------------+---------------------+---------|
|     14 | 12.17 | Mock Exam                      |                                                     |                     | EXAM    |
|--------+-------+--------------------------------+-----------------------------------------------------+---------------------+---------|
...
**** Pen-and-paper
- Reminder of probability and expectation: for a general random
  variable $x$ on a small outcome space with unspecified 
  probabilities, derive conditional expectations and apply to simple examples.
- Some things you can compute easily: expected height if you are man vs woman. 
- Some things are not obvious: what is the 'likelihood' that you are a man given your height?
- Given a list of problem descriptions, classify each as
  classification, regression, or neither, and identify the
  input/output variables.


**** In-class coding
- Compute a sample mean on a small dataset and compare it against the symbolic formula above.
- Include conditional probabilities and expectations on the given tables from the class data.
...
**** Pen-and-paper
- BSc: Given 1-2 datasets, draw a line through the two classes. 
- BSc: Write the hyperplane in the form Ax = b
- MSc: Write a formula for calculating the number of mistakes that a hyperplane Ax = b does.
- MSc: Chain rule test: Alternative cost functions for the perceptron, derive the update rule.
...
- In this assignment we compare classification models. We apply the
  ...
  another classifier.
- MSc: Chain rule test: Try it and see what happens.
...
- Examples of x/y variables and if they are C or R.

*** Lab 5: Basic neural networks

**** Pen-and-paper
- For two linear layers $z = Ax$, $y = Bz$ with *general* matrices $A,
  B$, show that $y = Cx$ for some matrix $C$, stacking linear layers
  (i.e. combinging two linear layers) makes no difference. Why does
  the argument break once a nonlinearity sits between the two layers?
- [Advanced: Multi-task case]
...
*** Exercises: Gradient methods and probability
*** Lab 8: Neural networks: advanced (can be done at 6)
...
The course grade is determined by a semester-long project evaluated
through continuous milestones and an individual oral
verification. While external tools may be used for implementation, they should be cited upon every use. 
Grading is strictly based on individual conceptual mastery and defense of the submitted work.
...
The written project report serves as technical documentation but does
not independently guarantee passing or final marks. It functions as
the baseline text for an individual oral examination. Students will be
questioned directly on the mathematical foundations, algorithmic
trade-offs, and codebase architecture submitted in the report. Final
grades depend heavily on the student's ability to independently
explain, justify, and modify their design decisions in real time
during the presentations and defense to maintain academic
integrity. Inability to explain the mechanisms within the report or
code can result in a failing grade for that component, regardless of
the software's execution status, or how polished the report is.
...
*** E (4)
1. Submission of a report from which one can obtain a high-level idea of their work without referring to their code. There might be serious ambiguities in all of the described methodology. The oral defense only provides partial explanations.
2. Technical correctness of their analysis with very little discussion. Possibly major errors in only a part of the implementation,  the students show some elementary understanding of the material during the oral.
...
1. The report/oral defense does not adequately explain their work.
2. There is very little discussion and major parts of the analysis are technically incorrect, or there are errors in the implementation, or oral explanations.
...
All non-programming exercises in the following chapters of the book 
may appear in some form in the exam
...
- Ch. 3 (3.1, 3.2, 3.5, 3.6)
- Ch. 4 (4.1, 4.3, 4.6, 4.7.4) 
- Ch. 5 (5.1,  5.3, 5.4, 5.5) 
- Ch. 7 (7.1, 7.2, 7.3, 7.5.3)
- Ch. 8 (8.1, 8.2, 8.4)
- Ch. 13 (13.1, 13.2, 13.2, 13.3)
- Ch. 16 (16.1)
...
** Differential privacy (not 2025/26)
...
** Markov decision processes and experiment design (not 2025/26)
