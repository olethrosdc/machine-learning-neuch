...
The likelihood of the observed data under our decision rule is
\[
\prod_{t=1}^T \pi(y_t | x_t).
\]
Let us call
\[
\hat{y}_t = \argmax_y \pi(y \mid x_t)
\]
the *predicted* or *most likely* value.
When $\pi(a | x)$ has a Gaussian form, then the mean-square error between $\hat{y}_t$ and $y$ is related to the negative likelihood of the Gaussian model, as we will see later.
...
- For all $\pi$, $\E_P[U(\pi, D_{\textrm{test}})] = U(\pi, P)$ (*unbiased* estimator)
...
#+ATTR_BEAMER: :overlay <+->
- $U(\mathcolor{blue}{\pi}, \mathcolor{green}{D})$ is the measured accuracy of a {{{color(blue,classifier $\pi$)}}}  on {{{color(green, data $D$)}}}
- $U(\mathcolor{blue}{\pi}, \mathcolor{red}{P}) = \E_{D \sim P} [U(\pi, D)]$ is the *actual* accuracy.
...
- Then $U(\mathcolor{magenta}{\lambda}(D), D)$ is biased, as for any $\pi'$
\begin{align}
\uncover<4->{
\E_{D \sim P} [U(\mathcolor{magenta}{\lambda}(D), D)]
 &= \int_{D} dP(D)  U(\mathcolor{magenta}{\lambda}(D), D)\\
}
\uncover<5->{
...
- $f_D = \sum_{t=1}^T (\mathcolor{magenta}{\lambda} + x_t) / T$.
