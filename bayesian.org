...
- We then observe $x_{t+1}$ and calculate a posterior distribution $\bel(\mu | x_1, \ldots, x_t, x_{t+1})$.
...
| Multinomial | Dirichlet  | $[0,1]^n$                       | $\{1, \ldots, n\}^T$  |
...
If $x_t \mid \param \sim \Ber(\param)$ then
\[
P_\param(x_t = 1) = \param
\]
*** Definition of the Beta density 
If $\param \sim \Beta(\alpha_1, \alpha_0)$ then
\[
...
If $x_t \mid \vparam \sim \Mult(\vparam)$ then
\[
P_\vparam(x_t = i) = \param_i
\]
*** Definition of the Dirichlet density 
If $\vparam \sim \Dir(\vectorsym{\alpha})$ then
\[
...
** Discrete Bayesian Networks
- A graphical model defined on variables $x_1, \ldots, x_n$ with each $x_n$ taking a finite number of values.
- We saw some simple such models for e.g. the cancer/smoking/asbestos example.
- Another example are discrete Markov models
*** Markov model
A Markov model obeys
\[
P(x_{k+1} | x_k, \ldots, x_1) = P(x_{k_1} | x_k)
\]
i.e. the graphical model is a chain
