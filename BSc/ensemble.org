...
* Ensemble methods
** General idea
- Concept class $\Pi$ (e.g. linear classifiers)
- Base learner $\lambda : \mathcal{D} \to \Pi$ (e.g. perceptron)
*** General Algorithm  
At iteration $b$,
1. Generate data $D_b$ from $D$.
2. Get base predictor $\pi_b = \lambda(D_b)$

At the end, combine predictors $\pi_1, \ldots, \pi_B$.
...
[[https://arxiv.org/abs/2212.02264][Bagging is an Optimal PAC learner]], Larsen 2024.
...
*** Algorithm
    - Input: Data $D$, number of experts $B$, base learner $\lambda$
    - For $B = 1, \ldots, B$
    -- Sample \alert{without replacement} $D_b \sim \Unif(D)$
    -- Obtain predictor $\pi_b = \lambda(D_b)$.
    - Return $\{\pi_b\}$
#+BEAMER: \pause
