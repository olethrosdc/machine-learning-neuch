...
- Mean: models the expected value of the population
- Variance: models the variance of the population
- Empirical distribution: models the distribution of the population
**** The expected value and the mean
Assume $x_t : \Omega \to \Reals$, and $\omega \sim p$
\[
\E[x] 
= \int_{-\infty}^\infty x(\omega) p(\omega) d\omega
\approx
