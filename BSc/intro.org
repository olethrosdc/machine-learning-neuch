...
- Each project should have a brief report explaining their methodology.
**** Key points
...
We use probability to model *randomness* and *uncertainty*

**** Elements of probability                                          :BMCOL:
:PROPERTIES:
:BEAMER_col: 0.5
:END:
#+ATTR_BEAMER: :overlay +-
- Random outcomes $\omega \in \Omega$
- An *event* $A \subseteq \Omega$ occurs if $\omega \in A$.
- *Probability* $P(A)$ measures the likelihood of events $A$.
- If $P(A) > P(B)$, then $A$ is more likely than $B$.
- For finite $\Omega$, we can use $P(\omega)$ instead of $P(\{\omega\})$.
**** Diagram                                                          :BMCOL:
:PROPERTIES:
:BEAMER_col: 0.5
:END:
\begin{tikzpicture}
\node (Omega) at (0,1)   [draw, circle, minimum size=2cm]  {};
\end{tikzpicture}
