...
**** Inconsistent data                                              :notes:
You can see that the entries are inconsistent. In some cases we have Male, and others M. The value N maybe a typo, or may indicate Neutral? The height and weight seem to be in cm and kg for most people, but Joan seems to use feet'inches and pounds.
...
- Check the histograms for inconsistnet raw values: does anything look wrong?
...
#+BEGIN_SRC python
  import numpy as np
  y = np.random.gamma(170, 1, size=20)  # 20 fresh samples -- not the class data
  y.mean()
  np.mean(y)
#+END_SRC
...
**** Probability space
- Defined on subsets of $\Omega$, a random *outcome space*. 
- Subsets $A$ of $\Omega$,  $A \subseteq \Omega$, are called *events*.

**** Probability axioms
A probability $P$, is a function from subsets\footnote{Formally, an algebra} of $\Omega$ to $[0,1]$ and it functions like an area:
- $P(\Omega) = 1$ (whatever happens must be in $\Omega$).
- $P(A) \geq 0$ is the *probability* of $A \subseteq \Omega$.
- If $A, B$ are *mutually exclusive* ($A \cap B = \emptyset$), then:
\[
P(A \cup B) = P(A) + P(B).
\]
*** Meaning of proability
...
*** Special cases
The outcomes or universe $\Omega$ depends on what we are interested in, and how we generate the random outcomes.
**** Discrete $\Omega$
- Consider throwing two coins. Then possible outcomes are
  $\Omega = \{HH, HT, TH, TT\}$ for heads or tails for each coin.
- Consider throwing one die. Then possible outcomes are
 $\Omega = [6] = \{1, 2, 3, 4, 5, 6\}$.
- Consider selecting an individual in Switzerland by randomly generating an AVS number: $\Omega = [10^{12}]$.

**** Continous $\Omega$
Consider throwing a die inside a $1 \times 1$ box. Then possible outcomes are in
$\Omega = [0,1]^2\times[0,360]^3$, because we need:
...
Even though the most natural outcome space for a die throw is $[6]$, we actually observe the die as a 3D object in $[0,1]^2\times[0,360]^3$. So we *map* the outcome in $\Omega$ onto $[6]$ if we want to play dice, i.e. we construct a *random variable (RV)* $x: [0,1]^2\times[0,360]^3 \to [6]$.

**** Dice game
- Here consider $\Omega = [6]$ and $P(\omega) = 1/6$ for every outcome $\omega \in \Omega$.
- Define the RV $x: \Omega \to \{0,1\}$ as
  \[
  x(\omega) = \begin{cases} 1, &\textrm{if $\omega >= 5$}\\
  0, &\textrm{if $\omega <5$}
  \end{cases}
  \]
...
:BEAMER_env: note
:END:
- An equivalent definition: $x(\omega) = \ind{\omega \geq 5}$,
uses the indicator function $\ind{}$ which takes the value $1$ when its argument is true and $0$ otherwise.

**** Probabilities of RVs
For any $x : \Omega \to X$, we can introduce a new probability measure $P_x$ so that, for any set of outcomes $S \subseteq X$ of the *random variable*, we have:
\[
P_x(S) = P(\cset{x(\omega) \geq 5}{\omega \in \Omega})
\]
...
:BEAMER_env: note
:END:
Since we are only interested in $x = 1$, then $S = \{1\}$. As a *shorthand*, we usually write $P_x(1)$ instead of $P_x(\{1\})$.
\[
P_x(1) = P(\cset{x(\omega) = 1}{\omega \in \Omega})
\]
Here, $\cset{x(\omega) = 1}{\omega \in \Omega}$ is the set of values $\omega \in \Omega$ for which $x(\omega) = 1$. These are $\{5, 6\}$ in our example.
Consequently,
\[
...
**** Exercises
- What are the expected winnings if you play this game? (on paper)
- Implement a method in python that generates draws from this random variable, using a uniformly random outcomes in $[0,1]$ as an input.
**** Solution
:PROPERTIES:
:BEAMER_env: note
:END:
...
- $\E_P[f]$ where $f(x,y) = x + y$.
- $\E_P[g]$ where $g(x,y) = xy$.
