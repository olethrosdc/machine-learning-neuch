...
- We usually do not deal with probability spaces directly. We use them as the underlying 'source of randomness' and instead look at *random variables*, functions $\Omega \to \Reals$
- Probabilities are usually subjective (exception: quantum physics).

*** Special cases
**** Implication
If $A \subset B$, then, which of the following is true?
- P(A) = P(B)
- P(A) > P(B)
- P(A) < P(B)
**** Discrete $\Omega$
- Consider throwing two coins. Then possible outcomes are
 e.g. $\Omega = \{HH, HT, TH, TT\}$.
- Consider throwing one die. Then possible outcomes are
 e.g. $\Omega = \{1, 2, 3, 4, 5, 6\}$.
- Consider selecting an individual in Switzerland by randomly generating an AVS number:
$\Omega = [10^12]$.

**** Continous $\Omega$
- Consider throwing a die inside a $1 \times 1$ box. Then possible outcomes can be described as
