...
*** Problem Formulation vs. Model Fitting
**** The "Hammer and Nail" Fallacy
- Students often force every real-world problem into standard classification or regression because those algorithms are familiar.
- Choosing the algorithm before defining the decision context leads to useless models.

*** Counterexample: Earthquake Prediction
**** The Naïve Reduction (Classification)
- *Setup:* Predict if an earthquake occurs today: $y \in \{0, 1\}$.
- *Result:* An algorithm that always predicts $y=0$ achieves $>99.9\%$ accuracy. 
- *Failure:* The model is completely useless for risk mitigation or evacuation decisions.
  
**** The Correct Formulation
- Earthquakes are rare, continuous spatiotemporal events.
- *Better questions:* Given a sequence of earthquake events with locations, what is the probability distribution of earthquake magnitudes at *all* possible locations tomorrow? Next week? Within a month?
- What is the expected utility or cost of an evacuation decision based on an imperfect warning pipeline?

** Variables
Let us give an example: Does gender influence math performance?

*** Main variables
- Those are the main variables we are interested in. 
Example: gender, math grades (*which* grades is important)
*** Related variables
...
Example: talent, high school teacher influence, family encouragement, personal interests
...
Each hypothesis above is really a claim about how the *joint distribution* factorises. Let $g$ be gender, $m$ math grade, $t$ teacher encouragement.
- Case 1 (independence): $\Pr(g, m) = \Pr(g)\Pr(m)$.
- Case 2 (mediation through $t$): $\Pr(g, m, t) = \Pr(g)\Pr(t \mid g)\Pr(m \mid t)$, so $\Pr(g,m) = \sum_t \Pr(g)\Pr(t\mid g)\Pr(m \mid t)$.
- Case 3 (direct effect): $\Pr(g, m) = \Pr(g)\Pr(m \mid g)$.
**** Exercise :exercise:
Cases 2 and 3 both let gender affect math grades, yet they are different *claims* about the world. Without collecting any data: what additional variable, if measured, would let you tell the two cases apart? Write the factorisation that would result if *both* mediation and a direct effect were present.
