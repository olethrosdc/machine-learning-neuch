...
#+TAGS: activity advanced MSc definition exercise homework project example theory code
...
*** Joint distributions, multiple variables :MSc:
...
**** Conditional independence :B_definition:
     ...
     :END:
- We say that $x, y$ are *conditionally* independent given $z$ and write $x \indep y \mid z$ if and only if $\Pr(x, y \mid z) = \Pr(x \mid z) \Pr(y \mid z)$.
- The graphical model expresses this via *separation*: $x$ influences $y$ only through $z$.
...
**** Col B                                                            :BMCOL:
	...
	How about here?
*** Time-series: Markov chain                                      :advanced:
Graphical models can be used for time-series modelling and prediction. The basic idea is that preceding measurements only influence future measurements, so it is natural for the arrows to point the same way.

**** Markov chain                                                       :MSc:
A Markov chain is the simplest such model. The state of the world, at time $t$ ,$x_t$ is all we need to know to predict what will happen at the next time period $x_{t+1}$.
...
Consequently, the independence condition

\[
\Pr(x_{t+1} \mid x_t, x_{t-1}, x_{t-2}) = \Pr(x_{t+1} \mid x_t)
\]
holds.
*** Time series: Hidden Markov model
However convenient, an *observable* state of the world is never really achievable. If it exists, it gives rise to *visible* measurements that depend on the *hidden* state. However, the observable measurements $y_t$ do *not* have this independence structure.
**** Hidden Markov model
      \begin{tikzpicture}
        \node[RV, hidden] at (0,0) (x1) {$x_1$};
        \node[RV, hidden] at (1,0) (x2) {$x_2$};
        \node[RV, hidden] at (2,0) (x3) {$x_2$};
        \node[RV] at (0,1) (y1) {$y_1$};
        \node[RV] at (1,1) (y2) {$y_2$};
        \node[RV] at (2,1) (y3) {$y_2$};
        \draw[->] (x1)--(x2);
        \draw[->] (x2)--(x3);
        \draw[->] (x1)--(y1);
        \draw[->] (x2)--(y2);
...
** Conditional independence exercises
...
**** A Bernoulli RV
Here, $x | \param \sim \Ber(\param)$
\begin{tikzpicture}
\node[RV] at (2,0) (x) {$x$};
\node[RV,hidden] at (0,1) (mean) {$\param$};
\draw[->] (mean) to (x);
\end{tikzpicture}
...
- \(\param = (\param_1, \ldots, \param_k)\)
\begin{tikzpicture}
      \node[RV] at (2,0) (x) {$x$};
      \node[RV] at (0,0) (y) {$y$};
      \node[RV,hidden] at (2,1) (mean) {$\mu$};
      \node[RV,hidden] at (3,1) (variance) {$\sigma$};
      \node[RV,hidden] at (0,1) (choice) {$\param$};
      \draw[->] (y) to (x);
      \draw[->] (mean) to (x);
      \draw[->] (variance) to (x);
      \draw[->] (choice) to (y);
\end{tikzpicture}
- $y \mid \param \sim \Mult(\param)$
...
- $z_t \mid  \param \sim \Mult(\param_i)$, $\param \in \Simplex^k$
- $x_t | z_t = i \sim \Normal(\mu_i, \Sigma_i)$.
- $\Mult(\param)$ is *multinomial*
\[
\Pr(z_t = i \mid \param) = \param_i
\]
...
\[
p(x | \param, \mu, \Sigma) = \sum_{z \in [k]} p(x \mid \mu_z, \Sigma_z) P(z \mid \param).
\]
**** Col B                                                            :BMCOL:
     :PROPERTIES:
     :BEAMER_col: 0.4
...
\begin{tikzpicture}
      ...
      \node[RV,hidden] at (2,1) (theta) {$\param$};
      \draw[->] (c) to (x);
      \draw[->] (theta) to (c);
      \draw[->] (sigma) to (x);
      \draw[->] (mu) to (x);
...
** Gradient algorithms
*** Gradient ascent
In the following we use $\param$ for *all* the parameters of the Gaussian mixture model,
with $x = (x_1, \ldots, x_T)$ and $z = (z_1, \ldots, z_T)$
**** Objective function
One way to estimate $\param$ is through maximising the likelihood
$L(\param) = P(x | \param)$
**** Marginalisation over latent variable
However, we need to marginalise over all values $z$
\[
L(\param) = \sum_z P(z, x | \param)
\]
...
\[
\param^{(n+1)} = \param^{(n)} + \alpha \nabla_\param L(\param).
\]
*** Gradient calculation
Here we use the *log trick*: $\nabla \ln f(\param) = \nabla f(\param) / f(\param)$.
\begin{align}
\nabla_\param L(\param)
& = \sum_z \nabla_\param P(z, x \mid \param) 
\\
&= \sum_z  P(z, x \mid \param) \nabla_\param \ln P(z, x \mid \param)
\\
&= \sum_z  P(x \mid z, \param)P(z \mid \param) \nabla_\param \ln P(z, x \mid \param)
\\
&\approx \frac{1}{m} \sum_{i=1}^m P(x \mid z^{(i)}, \param) \nabla_\param \ln P(z^{(i)}, x \mid \param)
&&z^{(i)} \sim P(z  \mid \param)
\end{align}
...
- For $i = 1, \ldots, m$: $z^{(i)} \sim P(z  \mid \param^{(n)})$
- $d^{(n)} = \frac{1}{m} \sum_{i=1}^m P(x \mid z^{(i)}, \param) \nabla_\param \ln P(z^{(i)}, x \mid \param^{(n)})$
- $\param^{(n+1)} = \param^n + \alpha d^{(n)}$.
*** Gradients of the GMMs
For the Gaussian mixture model, we need to calculate the following gradient, with respect to $\param = (\param, \mu, \sigma)$.
\[
\nabla_\param \ln P(z, x \mid \param)
= \sum_{t=1}^T  \nabla_\param \ln P(z_t, x_t \mid \param)
= \sum_{t=1}^T  \nabla_\param \ln [P(z_t \mid \param) P(x_t  \mid z_t, \mu, \sigma)]
\]
The individual terms are:
\[
\nabla_\param \ln P(z_t | \param)  = \frac{1}{P(z_t | \param)} \nabla_\param  P(z_t | \param)
\qquad
\frac{\partial}{\partial \param_i} =  \frac{\ind{z_t = i}}{P(z_t | \param)}
\]
\[
\frac{\partial}{\partial \mu_i} \ln P(x_t  \mid z_t = i, \mu, \sigma) 
= - \frac{\partial}{\partial \mu_i}  \frac{(x_t - \mu_i)^2}{2\sigma_i^2}
= \sigma_i^{-2} (x_t - \mu_i) 
\]
So the algorithm
- Randomly assigns $z_t$ according to the current $\param$ estimate
- Moves the centers towards the mean indicated by the $z$.
** Expectation maximisation
...
\begin{align*}
\ln P(x | \alert{\param})
& = \sum_z G(z) \ln P(x | \alert{\param})
 = \sum_z G(z) \ln [P(x, z | \alert{\param}) / P(z | x, \alert{\param})]
\\
& = \sum_z G(z) [\ln P(x, z | \alert{\param}) - \ln P(z | x, \alert{\param})]
\\
& = \sum_z G(z) \ln P(x, z | \alert{\param}) - \sum_z G(z) \ln P(z | x, \alert{\param})
\\
& = \sum_z P(z | x, \param^{(k)}) \ln P(x, z | \alert{\param}) - \sum_z P(z | x, \param^{(k)}) \ln P(z | x, \alert{\param})
\\
& \geq \sum_z P(z | x, \param^{(k)}) \ln P(x, z | \alert{\param}) - \sum_z P(z | x, \param^{(k)}) \ln P(z | x, \param^{(k)})
\\
& = Q(\alert{\param} \mid \param^{(k)}) +\mathbb{H}(z \mid x, \param^{(k)}),
\end{align*}
where 
\[
\mathbb{H}(z \mid  x, \param^{(k)})
= 
\sum_z  P(z \mid  x, \param^{(k)}) \ln P(z \mid x, \param^{(k)})
\]
is the entropy of $z$ for a fixed $x, \param^{(k)}$. As this is not negative, $\ln P(x | \param) \geq Q(\param \mid \param^{(k)})$.
...
\qquad
f(\vparam^{(k)}) = Q(\vparam^{(k)} | \vparam^{(k)}),
\]
...
** Bayesian Inference
*** Bayesian inference for the GMM
We saw, for the Beta-Bernoulli and Dirichlet-Multinomial models, that a *conjugate prior* turns Bayesian inference into a simple parameter update. The same idea extends to the GMM, by placing a prior on every parameter.
**** Col A                                                            :BMCOL:
     :PROPERTIES:
     :BEAMER_col: 0.4
     :END:
\begin{tikzpicture}
\node[RV] at (0,1) (x) {$x_t$};
\node[RV,hidden] at (0,0) (c) {$z_t$};
\node[RV,hidden] at (0,-1) (cat) {$\param$};
...
**** Col B                                                            :BMCOL:
     ...
     :END:
\begin{align}
\mu_i \mid \vectorsym{v} &\sim \Normal(\vectorsym{v}, I),\\
\Sigma_i \mid \vectorsym{W} &\sim \Wish(\vectorsym{W}),\\
\param \mid \vectorsym{\alpha} &\sim \Dir(\vectorsym{\alpha}),\\
z_t \mid \param &\sim \Mult(\param),\\
x_t \mid \mu, \Sigma, z_t = i & \sim \Normal(\mu_i, \Sigma_i).
\end{align}
**** Exercise :exercise:
Which of the five lines above, if any, would have a closed-form conjugate update if all the *others* were held fixed? Compare with the Beta-Bernoulli and Dirichlet-Multinomial cases covered earlier.
