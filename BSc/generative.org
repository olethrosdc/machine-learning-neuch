...
\theta \mid \vectorsym{\alpha} &\sim \Dir(\vectorsym{\alpha}),\\
z_t \mid \theta &\sim \Mult(\theta),\\
...
For the Gaussian mixture model, we need to calculate the following gradient, with respect to $\param = (\theta, \mu, \sigma)$.
\[
\nabla_\param \ln P(z, x \mid \param)
= \sum_{t=1}^T  \nabla_\param \ln P(z_t, x_t \mid \param)
= \sum_{t=1}^T  \nabla_\param \ln [P(z_t \mid \theta) P(x_t  \mid z_t, \mu, \sigma)]
\]
The individual terms are:
\[
\nabla_\theta \ln P(z_t | \theta)  = \frac{1}{P(z_t | \theta)} \nabla_\theta  P(z_t | \theta)
\qquad
\frac{\partial}{\partial \theta_i} =  \frac{\ind{z_t = i}}{P(z_t | \theta)}
\]
\[
...
\]
So the algorithm
- Randomly assigns $z_t$ according to the current $\theta$ estimate
