...
The total loss over the data is defined as $L(D, \param) = \sum_{(x, y) \in D} \ell(x, y, \param)$.
...
- $\frac{d}{dz} (\ind{z < 0} z) = \frac{d}{dz} \ind{z < 0} \times z +  \ind{z < 0} \times 1 = \ind{z < 0}$.\footnote{Note that for $z = 0$, there is no derivative but the subdifferential $[0,1]$.}
- $\frac{\partial}{\partial{\param_i}} [y(x_t^\top \param)] = y x_{t,i}$ (gradient of Perceptron's output)
- Gradient update: $\param^{t+1} = \param^t - \nabla_\param \ell(x_t,y_t, \param_t) = \param^t + y x_{t}  \ind{y x_t^\top \param^t  < 0}$ with $\alpha_t = 1$.
