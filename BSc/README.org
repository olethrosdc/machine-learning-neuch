...
**** Project (home)
- Sketch a candidate scientific question for your project, and identify one variable that would be central to answering it.
...
**** Pen-and-paper
- Without picking any specific dataset: argue why the *training* accuracy of $k$-NN is always $100\%$ when $k=1$, and why training accuracy can only get worse (or stay the same) as $k$ grows towards $T$.
- Given a generic utility matrix $U(y, a)$ (symbols, not numbers) and a model $P(y \mid x)$, derive the action $a^*$ maximising expected utility. Under what condition on $U$ does $a^*$ coincide with "predict the most likely class"? Give a $U$ for which it does not.
**** In-class coding
Here we look at different methods to measure model performance. Using scikit-learn, see how data rescaling affects performance. Then measure performance under the utility-based alternative above, instead of accuracy.
**** Project (home)
- Apply the same performance measure to a variable from your own project (real or simulated data).
...
**** Pen-and-paper
- Go through the mistake-bound proof line by line. For each of its two key inequalities ($\param^t \cdot \param^*$ grows by *at least* $\rho$ per mistake; $\|\param^t\|^2$ grows by *at most* $1$ per mistake), explain in words the geometric fact it captures.
- What step of the proof breaks down if the data is *not* linearly separable? What step breaks down if $\|x_t\|$ is unbounded?
**** In-class coding
In this assignment we compare classification models. We apply the basic ideas of train, test, validation, cross-validation and bootstrapping, using scikit-learn, to compare the perceptron against another classifier.
**** Project (home)
- Apply train/test or cross-validation to a variable from your own project.
