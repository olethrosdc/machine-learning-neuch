...
***  Setup
Choose on a utility matrix $U(y,a)$ for the spam example from the slides, by filling in the table:
|----------+------+------+-------|
| Utility  | Pass | Flag | Trash |
|----------+------+------+-------|
| Normal   |      |      |       |
...
*** Decisions versus predictions                                   :exercise:
- Once you have $a^* = \argmax_a \sum_y P(y|x) U(y,a)$, argue that no other decision rule can achieve higher expected utility under your model. 
- What changes once $P(y|x)$ is *estimated* (e.g. via kNN) rather than known exactly? Which new source of error are you now exposed to that wasn't there before?
- Show that if $Y=A$ and $U(y, a) = \ind{y = a}$, i.e. you only care about being right, then $a^* = \argmax_y P(y \mid x)$, i.e. the most likely class.
