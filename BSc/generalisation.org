...
This is the classical statistics formulation, applied to a regression estimator $f_D$,
 not generally for $U(\pol,P)$. It's good background, and the proof below is worth doing for the practice, but it's *optional*: the regret decomposition already covers what you need to know about generalisation error in this course.
...
#+END_EXPORT
**** Level notes
- *BSc*: Optional — skip unless you want the algebra practice.
- *MSc*: A clean derivation to be able to reproduce; every line uses only linearity of expectation and the i.i.d. noise assumption.
** Generalisation
...
*** Notes
(a) $\Pols^⊂\Pols\hat\Pols \subset \Pols$
\Pols^​⊂\Pols$ — what I drew. This is the only case where $U(\pol∗,P)≥U(\pol^∗,P)U(\pol^*,P)\geq U(\hat\pol^*,P)
U(\pol∗,P)≥U(\pol^​∗,P)$ is guaranteed: restricting your search space can only hurt or do nothing.

(b) $\Pols⊂\Pols^\Pols \subset \hat\Pols
\Pols⊂\Pols^​$ — e.g. comparing an overparameterized model (\Pols^\hat\Pols
\Pols^​) against a simpler reference class (\Pols\Pols
\Pols). Here "approximation error" can come out negative — the bigger class can only do as well or better.

(c) disjoint — two genuinely unrelated model families (e.g. trees vs. linear models, in general). No nesting at all, so there's no automatic ordering between $U(\pol∗,P)U(\pol^*,P)$
$U(\pol∗,P)$ and $U(\pol^∗,P)U(\hat\pol^*,P)$
$U(\pol^​∗,P)$ — you have to measure it.

(d) overlapping, neither contains the other — the realistic case for most real model comparisons. Same conclusion as (c): no free ordering.
...
\draw[thick,fill=gray!10] (0.9,-0.1) ellipse (1.9 and 1.3);
\node at (0.9,1.05) {\footnotesize $\hat\Pols$};
\filldraw (-1.8,0) circle (1.6pt);
...
\draw[dashed] (0.5,0.2) circle (1.1);
\draw[<->] (-1.8,0) -- (0.5,0.2);
...
**** Level notes
- *BSc*: Don't worry about reproducing this diagram — just be able to say, in your own words, which gap is "the model's fault" and which is "bad luck with the data," for a model you're using in your project.
- *MSc*: Notice the dashed circle is doing real work: it's the only *random* element in the picture. Convince yourself that the approximation error is a fixed number once the model class is fixed, while the estimation error is a random variable (a function of $D$) — that distinction is exactly what the Hoeffding bound below is quantifying.
*** Approximation error
