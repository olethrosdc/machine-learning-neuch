...
*** Gaussian model :B_example:exercise:
    :PROPERTIES:
    :BEAMER_env: example
    :END:
#+ATTR_BEAMER: :overlay <+->
- $x_t \in \Reals$, for $t \in [T]$, $\mu \in \Reals, \sigma \in \Reals_+$
- $x_t \mid \mu, \sigma \sim \Normal(\mu, \sigma)$
- Follow the *same steps* as for the Bernoulli model above — write the log-likelihood, differentiate with respect to *each* parameter separately, set both derivatives to zero — to show that
\[
\hat\mu_{\textrm{ML}}(x) = \frac{1}{T}\sum_{t=1}^T x_t,
\qquad
\hat\sigma^2_{\textrm{ML}}(x) = \frac{1}{T}\sum_{t=1}^T \left[x_t - hat\mu_{\textrm{ML}}(x)\right]^2.
\]
- Does this estimator for $\sigma^2$ have a problem analogous to the Bernoulli one above? (Hint: what happens when $T=1$?)
